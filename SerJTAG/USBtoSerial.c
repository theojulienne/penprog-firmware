...
TASK_LIST
{
	...
	{ .Task = PENPROG_Task         , .TaskStatus = TASK_STOP },
};
...
int main(void)
{
	// check if we're trying to get back to the bootloader
...
void EVENT_USB_ConfigurationChanged(void)
{
	...
	/* Setup PenProg JTAG endpoints */
	Endpoint_ConfigureEndpoint(PENPROG_TX_EPNUM, EP_TYPE_BULK,
		                       ENDPOINT_DIR_IN, PENPROG_TXRX_EPSIZE,
	                           ENDPOINT_BANK_SINGLE);

	Endpoint_ConfigureEndpoint(PENPROG_RX_EPNUM, EP_TYPE_BULK,
		                       ENDPOINT_DIR_OUT, PENPROG_TXRX_EPSIZE,
	                           ENDPOINT_BANK_SINGLE);
	...
	/* Start PENPROG task */
	Scheduler_SetTaskMode(PENPROG_Task, TASK_RUN);
}
...
TASK(CDC_Task)
{
	if (USB_IsConnected)
	{
		...
		*/
		...
		// also dont even bother unless we're ready to send
		if (Tx_Buffer.Elements && Endpoint_IsReadWriteAllowed())
		{
			/* Wait until Serial Tx Endpoint Ready for Read/Write */
			//while (!(Endpoint_IsReadWriteAllowed()));
			
			/* Write the bytes from the buffer to the endpoint while space is available */
			while (Tx_Buffer.Elements && (Endpoint_BytesInEndpoint() < CDC_TXRX_EPSIZE - 1)) // never fill it!
			{
